...
import { checkActive } from 'routes/sidebar'
import { IRoute, Role } from 'utils/types'
import SidebarContext from 'context/SidebarContext'
import { isSidebarItemActivate } from './SidebarContent'
...
function SidebarSubmenu({ route, role, linkClicked }: ISidebarSubmenu) {
  ...
  const [isDropdownMenuOpen, setIsDropdownMenuOpen] = useState(
    route.routes
      ? route.routes.filter((r) => {
        return isSidebarItemActivate(r, pathname)
      }).length > 0
      : false
  )
  ...
  return (
    <li className="relative px-6 py-3" key={route.name}>
      {isDropdownMenuOpen && (
        <span
          className='absolute h-12 inset-y-0 left-0 w-1  rounded-tr-lg rounded-br-lg'
          aria-hidden='true'
        ></span>
      )}
      <button
          ...
          route.routes && route.routes.map((r) => (
            ...
            >
              <Link
                ...
                scroll={false}
                passHref
                className={`w-full inline-block ${isSidebarItemActivate(r, pathname)
                  ? 'dark:text-gray-100 text-gray-800'
                  : ''
                  }`}
                onClick={linkClicked}
